...
def _eval_single_fold_adaboost(model, X_train, y_train, X_val, y_val):
...
def train_and_eval_adaboost(
    data_path=os.path.join(PROJECT_ROOT, "data", "filtered_processed_games.csv"),
    n_splits=10,
    n_jobs=-1
):
    print("=" * 70)
    print(" [03_CLASSIFICATION] ADABOOST CLASSIFIER (10-FOLD CV - TỐI ƯU SONG SONG)")
    print("=" * 70)
    ...
    fold_results = Parallel(n_jobs=n_jobs)(
        delayed(_eval_single_fold_adaboost)(
            ada,
    ...
    print(f"[+] AdaBoost ({n_splits} Folds CV, n_jobs={n_jobs}):")
    ...
    ada.fit(X, y)
    model_save_path = os.path.join(PROJECT_ROOT, "models", "adaboost.joblib")
    os.makedirs(os.path.dirname(model_save_path), exist_ok=True)
    joblib.dump(ada, model_save_path)
